...
class PlotPPC2(Node):
    def __init__(self):
        ...
        self.current_waypoint_idx = 1  # 원본 waypoints 인덱스 (활성 선분 결정용)
    ...
    def waypoint_idx_callback(self, msg: Int32):
        ...
        # 미션 완료 감지: 정확히 마지막 waypoint 인덱스에 도달했을 때
        # (ppc_2.py의 check_arrival에서 mission_completed=True 설정 시 idx를 len-1로 설정함)
        if self.waypoints and self.current_waypoint_idx == len(self.waypoints) - 1:
            # 이전 인덱스와 다르고(방금 도달), 아직 저장 안했고, 활성 상태일 때만 저장
    ...
    def setup_plot(self):
        ...
        # [NEW] 활성 선분 강조 표시 (굵은 마젠타)
        self.active_segment_line, = self.ax_path.plot([], [], 'm-', linewidth=4, 
                                                       label='Active Segment', alpha=0.7, zorder=3)
